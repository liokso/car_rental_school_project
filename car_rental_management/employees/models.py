...
class Employee(models.Model):
    ...
    employeecityid = models.ForeignKey('Citylist', models.DO_NOTHING, db_column='employeeCityID', blank=True, null=True)  # Field name made lowercase.
    ...
    employeeemailverify = models.CharField(db_column='employeeEmailVerify', max_length=40, blank=False, default='0')
...
class Salesorder(models.Model):
    ...
    pickupstoreid = models.ForeignKey('Store', models.DO_NOTHING, db_column='pickupStoreID', related_name='pickupStoreID', blank=True, null=True)  # Field name made lowercase.
...
class V_Cities(models.Model):
	mis_cityID = models.IntegerField(db_column='cityID',primary_key=True)
	mis_cityName = models.CharField(db_column='cityName',max_length=50)
	class Meta:
		managed = False
		db_table = 'v_cities'

class V_VehicleMake(models.Model):
	mis_makeID = models.IntegerField(db_column='rowNum',primary_key=True)
	mis_makeName = models.CharField(db_column='makeName',max_length=50)
	class Meta:
		managed = False
		db_table = 'v_vehiclemake'

class V_VehicleModel(models.Model):
	mis_modelID = models.IntegerField(db_column='rowNum',primary_key=True)
	mis_modelName = models.CharField(db_column='model',max_length=50)
	class Meta:
		managed = False
		db_table = 'v_vehiclemodel'	

class v_SalesOrder(models.Model):
	...
	makeName = models.CharField(db_column='makeName', max_length=50, blank=True, null=True) 
	model = models.CharField(max_length=50, blank=True, null=True)
	series = models.CharField(max_length=50, blank=True, null=True)
	seriesYear = models.IntegerField(db_column='seriesYear', blank=True, null=True)  
	engineSize = models.DecimalField(db_column='engineSize', max_digits=2, decimal_places=1, blank=True, null=True)  
	tankCapacity = models.DecimalField(db_column='tankCapacity', max_digits=4, decimal_places=1, blank=True, null=True)  
	power = models.DecimalField(max_digits=5, decimal_places=1, blank=True, null=True)
	seatingCapacity = models.IntegerField(db_column='seatingCapacity', blank=True, null=True) 
	wheelDrive = models.CharField(db_column='wheelDrive', max_length=3, blank=True, null=True)  
	wheelBase = models.DecimalField(db_column='wheelBase', max_digits=5, decimal_places=1, blank=True, null=True)
	transmission = models.CharField(db_column='transmission', max_length=5, blank=True, null=True)

	class Meta:
		managed = False
		db_table = 'v_salesorder'	