...
function html2pdf (html, pdf,callback) {
	...
	html = html.outerHTML
	// html2 = html.innerHTML
	//html2.remove("#condition_form")
	
	// html2 = html2.replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, '');
	// html2 = html2.replace(/<form\b[^<]*(?:(?!<\/form>)<[^<]*)*<\/form>/gi, '');
	// html2 = html2.replace(/<nav\b[^<]*(?:(?!<\/nav>)<[^<]*)*<\/nav>/gi, '');
	// html2 = html2.replace(/<button\b[^<]*(?:(?!<\/button>)<[^<]*)*<\/button>/gi, '');
	
	// console.log(html2)
	
	// html = html2
	// bodyCopy = html.cloneNode(true);
	// html2 = html
	// html2.hide('form')
	// console.log(html)
	//bodyCopy.remove("#condition_form");
	...
	if (typeof html === 'string') {
		...
		//html = html.replace(/<nav\bclass="navbar navbar-inverse">.*<\/nav>/gi, '');
		//html = html.replace(/<div class="tab">.*<\/div>/gi, '');
